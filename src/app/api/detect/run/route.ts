...
import { loadShippingData } from "@/core/detection/shipping-data-loader";
import { persistFindings } from "@/core/detection/findings-repository";
import type {
  RefundLeakPayload,
  ShippingLeakPayload,
} from "@/core/detection/finding-payload";
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  console.log(`[api/detect/run] Loading shipping data for ${shopDomain}`);
  const shippingData = await loadShippingData(shopId);
  console.log(
    `[api/detect/run] Loaded ${shippingData.orders.length} shipping orders, ` +
      `${shippingData.methodStats.length} method aggregates`,
  );
  ...
  for (const result of run.results) {
    ...
    try {
      if (result.detectorType === "refund_leak") {
      ...
      } else if (result.detectorType === "shipping_leak") {
        const payload = result.payload as ShippingLeakPayload;
        const persistResult = await persistFindings(
          shopId,
          result.detectorType,
          payload.findings,
        ...
        totalEvidenceRows += persistResult.evidenceRows;
        console.log(
          `[api/detect/run] Persisted shipping_leak findings: ` +
            `${persistResult.inserted} inserted, ${persistResult.updated} updated, ` +
            `${persistResult.evidenceRows} evidence rows`,
        );
      }
